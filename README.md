# CCSDS 235.1 Competition Planning Pack

This folder contains an execution-ready planning set based on the official deliverables document.

## Documents

- Program execution plan: [Program-Execution-Plan.md](./docs/Program-Execution-Plan.md)
- Phase 1 plan (Gateways 0-3): [Phase-1-Foundation-Core-Protocol.md](./docs/Phase-1-Foundation-Core-Protocol.md)
- Phase 2 plan (Workshop preparation): [Phase-2-Workshop-Preparation.md](./docs/Phase-2-Workshop-Preparation.md)
- Phase 3 plan (Gateways 4-6): [Phase-3-Session-Control-Integration.md](./docs/Phase-3-Session-Control-Integration.md)
- Phase 4 plan (Gateways 7-10): [Phase-4-Validation-Documentation.md](./docs/Phase-4-Validation-Documentation.md)
- IDA and blueprint starter pack: [IDA-Blueprint-Pack.md](./docs/IDA-Blueprint-Pack.md)

## Team Assumptions

- Team size: 4
- Roles: 2 engineers + 2 programmers
- Objective: maximize score (100/100 target), while maintaining deterministic protocol behavior suitable for mission-grade reliability.

## How To Use This Pack

1. Review `Program-Execution-Plan.md` and assign real names to role placeholders.
2. Use each phase file as the sprint and gateway control plan.
3. Use `IDA-Blueprint-Pack.md` to produce architecture artifacts and review them at each gateway.
4. Track progress weekly against gateway validation checklists and scoring categories.
